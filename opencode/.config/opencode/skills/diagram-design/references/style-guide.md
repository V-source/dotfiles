...
|---|---|---|---|
| `paper` | Page background, default node fill | `#f5f5f7` (Apple bg) | `#161618` (Apple dark bg) |
| `paper-2` | Diagram container bg, secondary fill | `#ececf0` (chart grid) | `#1c1c1e` (elevation-1) |
| `ink` | Primary text, primary stroke | `#1d1d1f` (Apple text) | `#f5f5f7` (Apple light text) |
| `muted` | Secondary text, default arrow stroke | `#6e6e73` (Apple muted) | `#a1a1a6` (Apple dark muted) |
| `soft` | Sublabels, boundary labels | `#8a8a8e` (chart axis) | `#8e8e93` (dark axis) |
| `rule` | Hairline borders | `rgba(29,29,31,0.12)` | `rgba(245,245,247,0.12)` |
| `rule-solid` | Stronger borders, baselines | `#d2d2d8` (border-strong) | `#48484a` (dark border-strong) |
| `accent` | Focal / 1–2 max per diagram | `#0071e3` (Apple blue) | `#0a84ff` (Apple dark blue) |
| `accent-tint` | Fill for accent-bordered boxes | `rgba(0,113,227,0.08)` | `rgba(10,132,255,0.10)` |
| `link` | HTTP/API calls, external arrows | `#0066cc` (info blue) | `#64b5ff` (dark info blue) |

> **Brand palette source:** this skin was extracted from the **inventario** project design system (`src/tokens.css` — Apple-style tokens). Semantic mapping: `--color-bg-primary` → `paper`, `--color-chart-grid` → `paper-2`, `--color-text-primary` → `ink`, `--color-text-muted` → `muted`, `--color-chart-axis` → `soft`, `--color-border-strong` → `rule-solid`, `--color-accent` → `accent`, `--color-info` → `link`. Dark values come from `[data-theme="dark"]` (bg `#161618`, elevation-1 `#1c1c1e`, accent `#0a84ff`, info `#64b5ff`). The `rule`, `accent-tint`, and dark `rule` tokens are derived (ink-at-opacity and accent-at-opacity).
